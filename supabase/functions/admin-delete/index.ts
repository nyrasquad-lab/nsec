...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const deleterId = token.split(".")[0];

    const { admin_id } = await req.json();

    if (!admin_id) {
      return new Response(JSON.stringify({ error: "Admin ID required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    if (admin_id === deleterId) {
    ...
    await adminClient.from("audit_logs").insert({
      admin_id: deleterId, action: "admin_deactivated", entity_type: "admin", entity_id: admin_id,
    });
