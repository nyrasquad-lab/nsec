...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (message && message.trim()) {
      const statusChangeStr = status_changed && status !== oldStatus
        ? `${oldStatus} → ${status}`
        : null;
    ...
    const subject = `Update on your ticket ${ticketNumberStr} - ${statusLabels[status]}`;
      ...
      <div style="font-family: Inter, Arial, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px;">
        ...
        <div style="background: #f8fafc; padding: 24px; border: 1px solid #e2e8f0; border-radius: 0 0 12px 12px;">
          ...
          <p style="margin: 0 0 16px; font-size: 15px;">
            Your support ticket <strong>${ticketNumberStr}</strong> has been updated.
          </p>
          <table style="width: 100%; margin-bottom: 16px; font-size: 14px;">
            ...
            <tr><td style="color: #64748b; padding: 4px 0;">Status:</td><td style="font-weight: 600; padding: 4px 0; color: ${status === "resolved" ? "#10b981" : status === "closed" ? "#6b7280" : "#2563eb"};">${statusLabels[status]}</td></tr>
          </table>
          ${message && message.trim() ? `
          ...
          <p style="margin: 0 0 8px; font-size: 14px;">
            You can track your ticket anytime using your ticket number and email at our support portal.
          </p>
          <p style="margin: 0; font-size: 13px; color: #64748b;">
            Ticket number: <strong>${String(ticket.ticket_number).padStart(4, "0")}</strong><br>
            Email: ${ticket.email}
          </p>
        </div>
        <p style="text-align: center; font-size: 12px; color: #94a3b8; margin-top: 16px;">
          IT Support Hub &copy; 2025
        </p>
      </div>
    `;

    // Use Supabase's built-in email via the auth admin API
    // We send through the notification system
    try {
      await fetch(`${supabaseUrl}/functions/v1/send-notification`, {
        ...
        // Notification endpoint may not exist yet - that's OK
      });
    } catch {
